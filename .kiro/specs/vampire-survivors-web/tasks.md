...
- [x] 2.2 實作 Object Pool 系統（core/pool/）
...
- [x] 3.1 實作 BootScene（scenes/BootScene.ts）
  - [x] 3.1.1 preload() 載入 Texture Atlas、Tilemap JSON、音效檔案、JSON 設定檔
  - [x] 3.1.2 建立載入進度條 UI
  - [x] 3.1.3 create() 完成後切換至 MainMenuScene

- [x] 3.2 實作 MainMenuScene（scenes/MainMenuScene.ts）
  - [x] 3.2.1 顯示主選單按鈕（開始遊戲、角色選擇、永久升級、設定）
  - [x] 3.2.2 實作角色選擇畫面（顯示角色列表、屬性、解鎖狀態）
  - [x] 3.2.3 實作地圖選擇畫面
  - [x] 3.2.4 實作永久升級介面（5 種升級屬性、金幣扣除）
...
- [x] 6.3 實作武器升級與進化
  - [x] 6.3.1 實作 levelUp() 方法（依據 levelData 提升屬性，最高 8 級）
